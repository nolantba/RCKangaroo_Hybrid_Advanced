...
__global__ void KernelA(const TKparams Kparams)
{
    ...
    for (int step_ind = 0; step_ind < STEP_CNT; step_ind++)
    {
        ...
        for (int group = 0; group < PNT_GROUP_CNT; group++) {
            ...
            u16 jmp_ind_loc = (u16)((X[0] + herd_bias) & JMP_MASK);  // HERD BIAS ADDED
		...
		jmp_ind = (x[0] + herd_bias) & JMP_MASK;  // HERD BIAS ADDED
        ...
        for (int group = PNT_GROUP_CNT - 1; group >= 0; group--)
        {
			...
			jmp_ind = (x0[0] + herd_bias) & JMP_MASK;  // HERD BIAS ADDED
			jmp_table = ((L1S2 >> group) & 1) ? jmp2_table : jmp1_table;
			Copy_int4_x2(jmp_x, jmp_table + 8 * jmp_ind);
...
#if USE_R2_4V
	// No jmp1_table in LDS: J1 reads from Kparams.Jumps1 (3MB L2 cache, 192KB footprint)
	// lds_jlist starts at LDS[0] — frees 32KB shared mem → more blocks resident per SM
...
#if USE_R2_4V
			// Pack variant bits [10:9] into jmp_ind for KernelB distance recovery
...
#if USE_R2_4V
	if (d_cur & JMP2_FLAG) {
		// J2 dist: from LDS jmp2_d (unchanged)
		((int4*)(jmp))[0] = ((int4*)(jmp2_d + 4 * (d_cur & JMP_MASK)))[0];
		jmp[2] = *(jmp2_d + 4 * (d_cur & JMP_MASK) + 2);
	} else {
		// J1 4-variant: bits[10:9] = variant, bits[8:0] = jump index
		int r2v = (d_cur >> 9) & 3;
...
{
#if USE_R2_4V
	// J1 distances served from Kparams.Jumps1 (L2 cache) in ProcessJumpDistance
	// Only preload jmp2_d into LDS
	u64* jmp2_d = LDS; //16KB
	u64* jmp1_d = nullptr; // not used with 4-variant
	int i = THREAD_X;
	while (i < JMP_CNT)
	{
