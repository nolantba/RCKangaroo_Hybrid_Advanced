...
#endif
	// Initialize Lambda endomorphism for GLV optimization
	InitLambda();
};
...
// GLV-based multiplication using Lambda endomorphism (~40% faster)
EcPoint Ec::MultiplyG_Lambda(const EcInt& k) {
	return ::MultiplyG_Lambda(k);  // Call global Lambda function
}
...
EcPoint Ec::MultiplyG_WNAF4(const EcInt &k) {
